...
if (!function_exists('connect')) {
    ...
    function connect()
    {
        $con = new mysqli("localhost", "root", "", "turfmanagement");
...
function getTimeSlot($start_time, $end_time, $turfid,$userid,$turfname, $date,$con)
{
    ...
    while(strtotime($startTime) < strtotime($endTime)){
        ...
        $i++;var_dump($startTime);
        if(strtotime($startTime) <= strtotime($endTime)){
            $sql = "INSERT INTO slot (date, turfid, userid, turfname, starttime, closetime) VALUES ('$date','$turfid','$userid','$turfname','$start', '$end')";
...
function login($username, $password)
{
    ...
    $q = connect()->query("SELECT * FROM register WHERE email = '$username' AND password = '$password' AND status = '1' ")->num_rows;
    if ($q == 1) return 1;
    return 0;
}
...
{
    $q = connect()->query("SELECT * FROM register WHERE username = '$username' AND password = '$password' ")->num_rows;
    if ($q == 1) return 1;
    return 0;
}
...
{
    $conn = connect();
    $q = $conn->query("SELECT * FROM register WHERE userid = '$id'")->fetch_assoc();
    return $q['name'];
}
...
function uploadFile($file)
{
    ...
    $upload = move_uploaded_file(@$_FILES[$file]['tmp_name'], "uploads/" . $image);
    if ($upload) {
...
{
    $proof = genRand() . "." . strtolower(pathinfo(@$_FILES[$files]['name'], PATHINFO_EXTENSION));
    $valid_extension = array("jpg", "png", "jpeg");
    //Check for valid file size
    if (($_FILES[$files]['size'] && !in_array(strtolower(pathinfo(@$_FILES[$files]['name'], PATHINFO_EXTENSION)), $valid_extension)) || ($_FILES[$files]['size'] && $_FILES[$files]['error']) > 0) {
        return -1;
    }
    $upload = move_uploaded_file(@$_FILES[$files]['tmp_name'], "uploads/" . $proof);
    if ($upload) {
        chmod("uploads/" . $proof, 0777);
        return $proof;
    } else {
        return -1;
    }
...
function uploadTurfFiles($files)
{
 $timage = genRand() . "." . strtolower(pathinfo(@$_FILES[$files]['name'], PATHINFO_EXTENSION));
    $valid_extension = array("jpg", "png", "jpeg");
    //Check for valid file size
    if (($_FILES[$files]['size'] && !in_array(strtolower(pathinfo(@$_FILES[$files]['name'], PATHINFO_EXTENSION)), $valid_extension)) || ($_FILES[$files]['size'] && $_FILES[$files]['error']) > 0) {
        return -1;
    }
    $upload = move_uploaded_file(@$_FILES[$files]['tmp_name'], "uploads/" . $timage);
    if ($upload) {
        chmod("uploads/" . $timage, 0777);
        return $timage;
    } else {
        return -1;
    }
...
function uploadProfilePhoto($filez)
{

    $timage = genRand() . "." . strtolower(pathinfo(@$_FILES[$filez]['name'], PATHINFO_EXTENSION));
    $valid_extension = array("jpg", "png", "jpeg");
    //Check for valid file size
    if (($_FILES[$filez]['size'] && !in_array(strtolower(pathinfo(@$_FILES[$filez]['name'], PATHINFO_EXTENSION)), $valid_extension)) || ($_FILES[$filez]['size'] && $_FILES[$filez]['error']) > 0) {
        return -1;
    }
    $upload = move_uploaded_file(@$_FILES[$filez]['tmp_name'], "uploads/" . $timage);
    if ($upload) {
        chmod("uploads/" . $timage, 0777);
        return $timage;
    } else {
...
{
    $row = $conn->query("SELECT image FROM register WHERE userid = '$id'")->fetch_assoc();
    if (strlen($row['image']) < 10) return "images/trainlg.png";
    else return "uploads/" . $row['image'];
}
...
{
    $val = connect()->query("SELECT * FROM manager WHERE userid = '$id'")->fetch_assoc();
...
function querySchedule($type)
{
    ...
    $row = 0;
    if ($type == 'future') {
        $row = $conn->query("SELECT * FROM `slot` WHERE date(`date`,'%d-%m-%Y') >= STR_TO_DATE('$today','%d-%m-%Y')");
    } else {
        $row = $conn->query("SELECT * FROM `slot` date(`date`,'%d-%m-%Y') <= STR_TO_DATE('$today','%d-%m-%Y')");
    }
    return $row;
}

function getTurfFromSchedule($id)
{
    $q = connect()->query("SELECT turfid as id FROM schedule WHERE id = '$id'")->fetch_assoc();
    return getTurfPath($q['id']);
}
...
{
    $val = connect()->query("SELECT turf_name FROM schedule WHERE userid = '$id'")->fetch_assoc();
    return $val['turf_name'];
}
function alert($msg)
{
...
function printClearance($id)
{
    ...
    $uniqueCode = $row['code'];
    $turf = getTurfFromSchedule($row['turfid']);
    ...
    $t_image = $row['t_image'];
    $seat = $row['seat'];
    $turf = getTurfName($row['turf_name']);
    $class = $row['class'];
...
<tr><th><b>Turf:</b></th><td>$turf</td></tr>
...
<img weight="180" height="180" src="uploads/$image"></td>
...
function printReport($id)
{
    ...
    $sn = 0;
    $schedule = getTurfFromSchedule($id);
...
function sendFeedback($msg)
{
    ...
    $stmt = connect()->query("INSERT INTO feedback (userid, message) VALUES ('$me', '$msg')");
    if ($stmt) return 1;
    return 0;
}
...
{
    $me = $_SESSION['userid'];
    return connect()->query("SELECT * FROM feedback WHERE userid = '$me'");
}
...
function replyTo($id, $reply)
{
    ...
    $sql = $con->query("UPDATE feedback SET response = '$reply' WHERE f_id = '$id' ");
    if ($sql) return 1;
    return 0;
}
